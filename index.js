...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
    ...
    /* The nested function has access to its parent function's variables. */
...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(arr){
    const displayNames = [];
    arr.forEach(item => displayNames.push("name: " + item["animal_name"] + ", scientific: " + item["scientific_name"]));
    return displayNames;
  }
  ...
  */
  ...
  function lowerCaseNames(arr){
    const lowerCase = [];
    arr.forEach(item => lowerCase.push(item["animal_name"].toLowerCase()));
    return lowerCase;
  }
  ...
  */
  ...
  function USApop(arr){
    const getSum = [];
    arr.forEach(item => getSum.push(item["population"]));
    return Object.values(getSum).reduce((t,n) => t+n);
  }
